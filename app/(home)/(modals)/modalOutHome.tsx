...
export default function ModalOutHome() {
  const router = useRouter();
  const params = useLocalSearchParams();
  const { deleteHome } = useHomeStore();

  const handleConfirm = async () => {
    if (params.homeId) {
      await deleteHome(Number(params.homeId));
      router.back();
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <Text style={styles.title}>¿Eliminar {params.title || 'este elemento'}?</Text>
